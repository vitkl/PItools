##' domainEnrichment: filter domains by background domain count => foldEnrichmentDist => foldEnrichmentPval => pval correction => plot pvals => plot the distribution for a set of random proteins
...
##' @param frequency logical, choice of statistic: fold enrichment or frequency in a set (if TRUE - frequency)
##' @param proteinID protein ID for which to plot sampled distributions
##' @param pValPlot character specifying column name which contains p-values for histogram
##' @param remove_zeros logical, remove zero domain proteins from distribution before calculating p-vals?
##' @param calculate_qval logical, calculate q-value adjustment? Can fail if pvalue distribution is weird
##' @import data.table
##' @import qvalue
